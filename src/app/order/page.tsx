...
async function resolveAuthState(): Promise<AuthState> {
  ...
  // 3. Check for pending registration draft
...
export default async function MenuPage() {
  ...
  if (authState.state === "VERIFIED_SESSION") {
    // Fetch today's active menu for the ordering experience
...
async function getTodayMenu() {
  ...
  // Determine meal type by IST hour
  const istHour = ist.getUTCHours();
  const mealType = istHour < 15 ? "LUNCH" : "DINNER"; // Before 3pm IST → lunch
