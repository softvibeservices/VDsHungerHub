      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">
            Manage tiffin client companies
            {companies.filter((c) => c.status === "PENDING").length > 0 && (
              <span className="ml-2 inline-flex items-center gap-1 text-xs text-amber-700 bg-amber-50 border border-amber-100 rounded-full px-2 py-0.5 font-medium">
                <Clock size={10} /> {companies.filter((c) => c.status === "PENDING").length} pending OTP
              </span>
            )}
          </p>
        </div>
