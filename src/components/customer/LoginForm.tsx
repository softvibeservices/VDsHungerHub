    ...
    return (
      <div className="space-y-4">
        <div className="flex items-center gap-2 mb-2">
          <button type="button" onClick={() => { setForgotPin(false); setFpStep("mobile"); setError(""); }} className="text-gray-400 hover:text-gray-600 text-xs cursor-pointer">← Back</button>
        ...
        {fpStep === "mobile" && (
          <div className="space-y-3">
            ...
            <button id="fp-send-otp" type="button" onClick={handleForgotPinSendOtp} disabled={loading || mobile.length !== 10}
              ...
              {loading && <Loader2 size={14} className="animate-spin" />} Send Reset OTP
            </button>
          </div>
        )}
    ...
    <div>
      ...
      <form onSubmit={handlePinLogin} className="space-y-3">
        ...
        <div className="flex items-center justify-between text-xs pt-1">
          <button type="button" onClick={() => { setForgotPin(true); setError(""); }} className="text-orange-600 hover:underline font-semibold cursor-pointer">
            Forgot PIN?
          </button>
          <button type="button" onClick={onSwitchToRegister} className="text-gray-500 hover:text-gray-700 cursor-pointer">
